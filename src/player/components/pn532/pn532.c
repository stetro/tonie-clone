...
bool readdata(uint8_t *buff, uint8_t n) {
  ...
  for (uint8_t i = 0; i < (n + 2); i++) i2c_master_read_byte(i2ccmd, &buffer[i], I2C_MASTER_ACK);
...
#ifdef CONFIG_PN532DEBUG
  ESP_LOGD(TAG, "Received: 0x%.2X 0x%.2X 0x%.2X 0x%.2X 0x%.2X 0x%.2X 0x%.2X 0x%.2X", pn532_packetbuffer[0],
